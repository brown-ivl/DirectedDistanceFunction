...
Parser.add_argument('--val-rays-per-shape', help='Number of ray samples per object shape for validation.', default=1000, type=int)
...
if __name__ == '__main__':
    ...
    ValData = ODFDL(root=NeuralODF.Config.Args.input_dir, train=Args.force_test_on_train, download=True, n_samples=Args.val_rays_per_shape, usePositionalEncoding=usePosEnc)
    print('[ INFO ]: Training data has {} shapes and {} rays per sample.'.format(int(len(TrainData) / Args.rays_per_shape), Args.rays_per_shape))
    print('[ INFO ]: Validation data has {} shapes and {} rays per sample.'.format(int(len(ValData) / Args.val_rays_per_shape), Args.val_rays_per_shape))
