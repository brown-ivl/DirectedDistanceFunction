...
def extract_mesh_multiple_directions(Network, Device, resolution=256, ground_truth=None, show_curve=False):
    ...
    for i in range(len(all_depths)):
        ...
        #curr_intersection_mask = np.abs(all_depths[i]) < 0.01
    ...
    mask = stacked_depths<0.05#(2.*MESH_RADIUS/(resolution-1))*2
    stacked_depths[mask] = -1
    ...
    import open3d as o3d
    coordinates, bounds_mask = generate_marching_cubes_coordinates(resolution=resolution, direction=dir)
    print(coordinates.shape, stacked_depths.shape)
    data = coordinates.reshape((resolution, resolution, resolution, 6))
    pred_depth = np.stack(all_final_depths, axis=-1).reshape(-1, 1)
    #mask = (pred_mask>0.5).reshape(-1)
    mask = (np.abs(pred_depth)<0.05).reshape(-1)
    #mask = (np.abs(pred_depth)<2/191*2).reshape(-1)
    #mask = np.logical_and((pred_mask>0.5).reshape(-1), (np.abs(pred_depth)<0.05).reshape(-1))
    surf_pts = data[:, :, :, :3]+data[:, :, :, 3:]*np.stack(all_final_depths, axis=-1)
