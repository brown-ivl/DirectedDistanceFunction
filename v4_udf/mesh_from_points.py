...
def show_mesh(verts, faces, ground_truth=None):
    ...
    #mesh = mesh.filter_smooth_laplacian(number_of_iterations=20, filter_scope=o3d.geometry.FilterScope.Vertex)
...
def vis(data, gt_depth, gt_mask, pred_depth, pred_mask, data0):
    ...
    _, ind = pcd.remove_statistical_outlier(nb_neighbors=15, std_ratio=5.0)
    pcd = pcd.select_down_sample(ind)
    surf_pts =  np.array(pcd.points)
