...
def run_inference(Network, embeddings, z_index, Device, coordinates):
    all_depths = []
    all_masks = []

    Network.eval()

    # Append latent codes to the coordinates
    # stacked_latents = torch.vstack([Latent,]*coordinates.shape[0]).to(Device)
    # coordinates = coordinates.to(Device)
    # coordinates = torch.cat([stacked_latents, coordinates], dim=1)
    indices = torch.tensor([z_index,]*coordinates.shape[0])

    sigmoid = torch.nn.Sigmoid()

    batch_size = 5000
    for i in range(0, coordinates.shape[0], batch_size):
        DataTD = (v3_utils.sendToDevice(coordinates[i:i+batch_size, ...], Device), v3_utils.sendToDevice(indices[i:i+batch_size], Device))

        output = Network([DataTD], embeddings)[0]
...
def extract_mesh_multiple_directions(Network, embeddings, z_index, Device, resolution=256, ground_truth=None, show_curve=False):
    ...
    for dir in tqdm(directions):
        ...
        depths, intersection_mask = run_inference(Network, embeddings, z_index, Device, coordinates)
...
if __name__ == '__main__':
    ...
    Parser.add_argument('--resolution', help='Resolution of the mesh to extract', type=int, default=192)
    ...
    z_ind = 1
    z_ind = torch.tensor(z_ind).to(Device)
    z = lat_vecs(z_ind)
    
    # verts, faces, normals = extract_mesh(NeuralODF, Device, resolution=Args.resolution)
    verts, faces, normals = extract_mesh_multiple_directions(NeuralODF, lat_vecs, z_ind, Device, resolution=Args.resolution, ground_truth=gt_mesh, show_curve=Args.show_curve)
