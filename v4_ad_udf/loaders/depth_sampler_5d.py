...
class DepthMapSampler():
    ...
    def sample(self, TargetRays, RatioPositive=DEPTH_SAMPLER_POS_RATIO):
        ...
        if self.Aug:
            ...
            outToInDepths = (torch.rand(len(outToInCoor), 1)*0.001)
            ...
            shift = torch.rand(len(surf), 1)*0.001
            ...
            self.Coordinates = torch.vstack([self.Coordinates, outToInCoor, intSur])
            self.Intersects = torch.vstack([self.Intersects, outToInInt, intSurInt])
            self.Depths = torch.vstack([self.Depths, outToInDepths, intSurDepth])
            #print(self.Coordinates.shape, self.Intersects.shape, self.Depths.shape)
