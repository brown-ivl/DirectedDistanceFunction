...
def compute_opengl_rotation_matrix(viewpoints, scene_origin=np.asarray([[0.,0.,0.]])):
...
def get_viewpoint_samples(viewpoint_samples, selected_samples_count=50):
    
    viewpoint_samples_indices = np.arange(viewpoint_samples.shape[0])
    # np.random.shuffle(viewpoint_samples_indices)
    
    viewpoint_samples = viewpoint_samples[viewpoint_samples_indices[:selected_samples_count]]
    print(viewpoint_samples.shape)
    ...
    opengl_camera_pose_matrix = compute_opengl_rotation_matrix(viewpoint_samples)
    # print(ray_directions.shape)
    return opengl_camera_pose_matrix, viewpoint_samples#all_viewpoints, ray_directions
...
# %%
# unifrom sphere sampling: https://stackoverflow.com/questions/5408276/sampling-uniformly-distributed-random-points-inside-a-spherical-volume
...
def main(mesh_vertices, mesh_faces, obj_name, data_save_dir=None, val_start_counter=1000, num_viewpoints=1200):  
    
    # sample viewpoints on sphere
    sampled_sphere_points = sphere_sampler(num_viewpoints)
    opengl_camera_pose_matrix, viewpoint_samples = get_viewpoint_samples(sampled_sphere_points, selected_samples_count=sampled_sphere_points.shape[0])
    print(opengl_camera_pose_matrix.shape)
    ...
    if not os.path.exists(data_save_dir):
        os.system('mkdir -p ' + os.path.join(data_save_dir))
    ...
    for iter in range(0, min(num_viewpoints, opengl_camera_pose_matrix.shape[0])): 
        ...
        opengl_camera = pyrender.PerspectiveCamera(yfov=np.pi/2.0, aspectRatio=1, znear=0.05, zfar=3)
        scene = pyrender.Scene()
        scene.add_node(pyrender.Node(mesh=mesh))
        scene.add(opengl_camera, pose=camera_viewpoint_pose)
        r = pyrender.OffscreenRenderer(256, 256)
        _, depth_map = r.render(scene)

        plt.imshow(depth_map)
        plt.show()

        # invalid depth map and unprojected skinning weights have -1 values
        unprojected_3D_points, invalid_depth_mask = unproject(depth_map, camera_viewpoint_pose, opengl_camera.get_projection_matrix())
        #if iter == 0:
        ...
        if not os.path.exists(os.path.join(data_save_dir, obj_name, 'depth', data_split)):
            os.system('mkdir -p ' + os.path.join(data_save_dir, obj_name, 'depth', data_split))
...
data_save_dir = 'F:\\ivl-data\\rendered-data\\'
for obj_file in all_obj_files:
    obj_name = os.path.basename(obj_file).split('.')[0]
    print(obj_name, "obj_name")
    mesh_vertices, mesh_faces = load_object(obj_name + '.obj')
    if not "alligator" in obj_name:
        main(mesh_vertices, mesh_faces, obj_name, data_save_dir)
