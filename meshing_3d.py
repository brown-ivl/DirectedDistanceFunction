...
class MeshODF():
    ...
    def query_rays(self, points, directions):
        ...
        directions = directions[-points.shape[0]:]
        intersect = []
        depths = []
...
def sample_next_vertices(model, vertices, faces, probes, directions, radius, delta, first_sampling=False):
    ...
    if not first_sampling:
        probes = probes - (delta * directions[probes_offset:,:])
    # lines = np.concatenate([faces[:,:2], faces[:,1:], faces[:,[0,2]]], axis=0)
    with torch.no_grad():
    ...
    new_vertices = [vertices[i] for i in range(probes_offset)] + [probes[i-probes_offset] + directions[i]*model_depths[i-probes_offset] if model_depths[i-probes_offset] < np.inf else vertices[i] for i in range(probes_offset, vertices.shape[0])]
...
def make_model_mesh(model, initial_tessalation_factor=2, radius=1.25, focal_point=[0.,0.,0.], show=True, iterations = 3, delta=0.1):
    focal_point = np.array(focal_point)
