...
class DepthODFDatasetLoader(torch.utils.data.Dataset):
    ...
    def loadData(self):
        ...
        DatasetDir = os.path.join(v5_utils.expandTilde(self.DataDir), os.path.splitext(self.FileName)[0])
        if os.path.exists(DatasetDir) is False:
            DataPath = os.path.join(v5_utils.expandTilde(self.DataDir), self.FileName)
            if os.path.exists(DataPath) is False:
                if self.isDownload:
                    print('[ INFO ]: Downloading', DataPath)
                    v5_utils.downloadFile(self.DataURL, DataPath)
            ...
            with zipfile.ZipFile(DataPath, 'r') as File2Unzip:
                ...
                File2Unzip.extractall(v5_utils.expandTilde(self.DataDir))
