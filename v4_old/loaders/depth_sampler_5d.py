...
class DepthMapSampler():
    ...
    def sample(self, TargetRays, RatioPositive=DEPTH_SAMPLER_POS_RATIO):
        ...
        if self.Aug:
            ...
            self.Coordinates[:,:3][mask2] = nonIntCoor[:,:3]+nonIntCoor[:,3:]*torch.rand(len(nonIntCoor), 1)*3.0
            ...
            shift = self.Depths[mask1]*(torch.minimum(torch.rand(len(intCoor), 1)*1.5, torch.tensor(1.0)))
            ...
            shift = self.Depths[mask][::4]
            intSur = self.Coordinates[mask][::4]
            surf = intSur[:,:3]+intSur[:,3:]*shift
            look = torch.tensor(v3_utils.sphere_surface_sampler(len(surf)))
            intSur[:,:3] = surf+look*torch.rand(len(surf), 1)*1.75#0.5
            intSur[:,3:] = -look
            intSurInt = torch.ones(len(surf), 1)
            intSurDepth = torch.ones(len(surf), 1)*100
            #print(surf.shape, look.shape, intSur.shape)
            
            
            # augment non-intersecting points from a sphere
            nonIntSphere = torch.ones(self.Coordinates.shape)[::4]
            nonIntSphere[:,:3] = torch.tensor(v3_utils.sphere_surface_sampler(len(nonIntSphere), 1.1+np.random.rand()*0.15))
            nonIntSphere[:,3:] = torch.tensor(v3_utils.sample_hemisphere(nonIntSphere[:,:3]))
            nonIntSphereInt = torch.zeros(len(nonIntSphere), 1)
            nonIntSphereDepth = torch.ones(len(nonIntSphere), 1)*100
