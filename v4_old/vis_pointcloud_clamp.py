...
def run_inference(Network, Device, DataLoader):
    ...
    for i, (Data, Targets) in enumerate(DataLoader, 0):
        ...
        for j in range(7):
            ...
            pred_acc_depth += output[1].detach().cpu().numpy()
...
def vis(data, gt_depth, gt_mask, pred_depth, pred_mask, last_pred_depth):
    ...
    mask = (np.abs(last_pred_depth)<0.05).reshape(-1)
    mask = np.logical_and((pred_mask>0.5).reshape(-1), (np.abs(last_pred_depth)<0.05).reshape(-1))
    ...
    print(np.sum((gt_mask==1)==(np.abs(last_pred_depth)<0.05))/(gt_mask.reshape(-1).shape[0]))
    #print(np.sum((gt_mask==1)==(maskboth.reshape(-1, 1)))/(gt_mask.reshape(-1).shape[0]))
    print(np.sum((gt_mask==1)==(np.logical_and(pred_mask>0.5, np.abs(last_pred_depth)<0.05)))/(gt_mask.reshape(-1).shape[0]))
...
if __name__ == '__main__':
    ...
    data, gt_depth, gt_mask, pred_depth, pred_mask, last_pred_depth  = run_inference(NeuralODF, Device, DataLoader)
