...
class ODFSingleV3(torch.nn.Module):
    ...
    def __init__(self, input_size=6, n_layers=6, hidden_size=256, radius=1.25, pos_enc=True):
        ...
        #self.layernorm0 = nn.LayerNorm(32, elementwise_affine=False) 
        self.layernorm = nn.LayerNorm(hidden_size, elementwise_affine=False)
        #self.dropout_bot = torch.nn.Dropout(p=0.0)
    ...
    def forward(self, input):
        ...
        for b in range(B):
            if not self.pos_enc:
                ...
                x = v3_utils.positional_encoding_tensor(Input[b], L=4)
                #print(x.shape)
                BInput = v3_utils.positional_encoding_tensor(Input[b], L=4)
