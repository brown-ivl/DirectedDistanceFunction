...
from losses import MultiViewRayLoss, MultiViewLoss
...
def train(save_dir, name, model, optimizer, train_loader, val_loader, loss_history, hyperparameters, device, scheduler):
    ...
    if "train" in loss_history:
        ...
        all_losses = ["train", "val", "train_depth", "val_depth", "train_intersection", "val_intersection", "ssl", "ssl2"]
        if "constant" in arch:
    ...
    for e in range(epochs):
        ...
        for batch in tqdm(train_loader):
            ...
            train_ssl_loss = ssl_loss_fn(model, data, output, targets)
            all_train_ssl_losses.append(train_ssl_loss.detach().cpu().numpy())
            train_ssl2_loss = ssl2_loss_fn(model, data, targets)
            all_train_ssl2_losses.append(train_ssl2_loss.detach().cpu().numpy())
            train_loss = train_depth_loss + train_intersection_loss 
            #train_loss = train_intersection_loss
            #if e>18000:
                #train_loss += train_ssl_loss
            #    train_loss += train_ssl2_loss
            if 'constant' in arch:
        ...
        loss_history["ssl2"].append(np.mean(np.asarray(all_train_ssl2_losses)))
        ...
        loss_dict = {"train_loss": np.mean(np.asarray(all_train_losses)),
                    ...
                    "train_intersection_loss": 1*np.mean(np.asarray(all_train_intersection_losses)),
                    "val_intersection_loss": 1*np.mean(np.asarray(all_val_intersection_losses)),
                    "ssl": np.mean(np.asarray(all_train_ssl_losses)),
                    "ssl2": np.mean(np.asarray(all_train_ssl2_losses))
                    }
        if "constant" in arch:
...
if __name__ == '__main__':
    ...
    Parser.add_argument('--batch-size', help='Choose mini-batch size.', required=False, default=96, type=int)
    ...
    ValData = DDL(root=Args.input_dir, name=Args.dataset, train=Args.force_test_on_train, download=True, target_samples=Args.val_rays_per_shape, usePositionalEncoding=Args.use_posenc, aug=True)
    ...
    ValDataLoader = torch.utils.data.DataLoader(ValData, batch_size=100, shuffle=True, num_workers=nCores, collate_fn=DDL.collate_fn)
